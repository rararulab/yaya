...
- **BDD contracts via [`ZhangHanDong/agent-spec`](https://github.com/ZhangHanDong/agent-spec).** Every non-trivial feature PR ships with `specs/<slug>.spec` (Intent · Decisions · Boundaries · Completion Criteria). Each scenario binds to a `tests/bdd/features/<slug>.feature` scenario and to pytest-bdd step defs in `tests/bdd/test_<module>.py` — real execution, not naming convention. `agent-spec lifecycle` + `scripts/check_feature_sync.py` gate this in CI and pre-commit. New agents authoring or migrating a spec follow [docs/dev/bdd-workflow.md](docs/dev/bdd-workflow.md) step-by-step — do not improvise. See also [docs/dev/agent-spec.md](docs/dev/agent-spec.md) for the spec authoring layer.
