...
1. Land commits on `main` using [Conventional Commits](https://www.conventionalcommits.org/) (`feat:`, `fix:`, etc.).
2. release-please opens a "chore: release X.Y.Z" PR that bumps `pyproject.toml` and updates `CHANGELOG.md`.
3. Merging that PR creates a git tag, which triggers the release workflow to build binaries + wheels and publish a GitHub Release with checksums.
