...
- Session single-read + provider check (#163): `GET /api/sessions/{id}` returns the full `SessionInfo` row plus `provider_availability: {available, active, known_providers}`. `available` is computed against `ProvidersView.list_instances()` — config-only check; runtime plugin load state is NOT consulted (documented caveat). `available=null` means no historical anchor (legacy tape) — UI suppresses the banner. On resume, chat-shell fetches this endpoint BEFORE hydrating and renders an inline `role="status"` banner above the chat pane when `available=false`. Two buttons: Continue (hydrates + opens WS with the currently-active provider) and Cancel (resets state, hash back to `#/chat`). No silent provider switch. A monotonic `resumeGeneration` counter guards `onResumeSession` so rapid sidebar clicks cannot cross-pollinate state between sessions.
