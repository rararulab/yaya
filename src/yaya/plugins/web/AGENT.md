...
- Session history endpoint (#159): `GET /api/sessions/{id}/messages` returns `{messages: [{role, content}, ...]}` — the same projection the agent loop applies each turn (`project_entries_to_messages` in `yaya.kernel.loop`). Tool calls do not replay in history view; the ReAct strategy records tool observations as user `"Observation: ..."` messages, which render as plain bubbles. Compaction anchors elide prior entries and inject the summary as a `system` row. 404 on unknown id, 503 when the store is not wired.
