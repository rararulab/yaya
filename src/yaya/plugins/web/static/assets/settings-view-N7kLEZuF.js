import{b as r,A as u,i as I,r as d,t as R}from"./index-KQ7Oknwq.js";class p extends Error{constructor(e,a,n=null){super(a),this.status=e,this.detail=n}}async function f(t,e,a){const n={method:t,headers:{Accept:"application/json"}};a!==void 0&&(n.body=JSON.stringify(a),n.headers={...n.headers,"Content-Type":"application/json"});const s=await fetch(e,n);if(!s.ok){let i=null;try{const l=await s.clone().json();l&&typeof l.detail=="string"&&(i=l.detail)}catch{}const h=i??`${t} ${e} → ${s.status}`;throw new p(s.status,h,i)}if(s.status!==204)return await s.json()}async function E(){const t=await f("GET","/api/plugins");return Array.isArray(t)?t:t.plugins??[]}function F(t,e){return f("PATCH",`/api/plugins/${encodeURIComponent(t)}`,e)}function P(t,e=!1){return f("POST","/api/plugins/install",{source:t,editable:e})}function T(t){return f("DELETE",`/api/plugins/${encodeURIComponent(t)}`)}function A(){return f("GET","/api/config")}function w(t,e=!1){const a=e?"?show=1":"";return f("GET",`/api/config/${encodeURIComponent(t)}${a}`)}function D(t,e){return f("PATCH",`/api/config/${encodeURIComponent(t)}`,{value:e})}function O(t){return f("DELETE",`/api/config/${encodeURIComponent(t)}`)}function C(t=!1){return f("GET",`/api/llm-providers${t?"?show=1":""}`)}function _(t){return f("POST","/api/llm-providers",t)}function L(t,e){return f("PATCH",`/api/llm-providers/${encodeURIComponent(t)}`,e)}function N(t){return f("DELETE",`/api/llm-providers/${encodeURIComponent(t)}`)}function j(t){return f("PATCH","/api/llm-providers/active",{name:t})}function U(t){return f("POST",`/api/llm-providers/${encodeURIComponent(t)}/test`)}function G(t){return t.length<3||t.length>64?!1:/^[a-z0-9][a-z0-9-]*[a-z0-9]$/.test(t)}const J=["_key","_token","_secret","_password"];function W(t,e){if(e?.format==="password")return!0;const a=t.toLowerCase();return J.some(n=>a.endsWith(n))}function v(t){const{schema:e,values:a}=t;if(!e||!e.properties)return S(t);const n=Object.entries(e.properties);return n.length===0?S(t):r`
	...
	`}function B(t,e,a,n){const s=e.title??t,i=e.description;return r`
		<label class="yaya-form-field">
			...
			${i?r`<span class="yaya-form-desc">${i}</span>`:u}
			${k(t,e,a,n)}
		</label>
	`}function k(t,e,a,n){const s=e.type??x(a);if(s==="boolean")return r`<input
			type="checkbox"
			.checked=${!!a}
			@change=${y=>n.onChange(t,y.target.checked)}
		/>`;if(s==="integer"||s==="number")return r`<input
			type="number"
			step=${s==="integer"?"1":"any"}
			.value=${a==null?"":String(a)}
			@change=${y=>{const g=y.target.value;if(g==="")return;const m=s==="integer"?Number.parseInt(g,10):Number.parseFloat(g);Number.isNaN(m)||n.onChange(t,m)}}
		/>`;if(s==="array"||s==="object"){const y=a===void 0?"":JSON.stringify(a,null,2);return r`<textarea
			rows="4"
			.value=${y}
			@change=${g=>{const m=g.target.value;try{n.onChange(t,JSON.parse(m))}catch{}}}
		></textarea>`}const i=W(t,e),h=n.revealSecrets.has(t),l=i&&!h?"password":"text",$=a==null?"":String(a);return r`<span class="yaya-form-row">
		<input
			type=${l}
			.value=${$}
			@change=${y=>n.onChange(t,y.target.value)}
		/>
		${i?r`<button
					type="button"
					class="yaya-reveal"
	...
	</span>`}function S(t){const e=Object.entries(t.values);return e.length===0?r`<p class="yaya-empty">No configuration fields available.</p>`:r`
	...
	`}function H(t){const e=x(t);return e===void 0?{}:{type:e}}function x(t){return typeof t=="boolean"?"boolean":typeof t=="number"?Number.isInteger(t)?"integer":"number":Array.isArray(t)?"array":t!==null&&typeof t=="object"?"object":"string"}var K=Object.defineProperty,z=Object.getOwnPropertyDescriptor,c=(t,e,a,n)=>{for(var s=n>1?void 0:n?z(e,a):e,i=t.length-1,h;i>=0;i--)(h=t[i])&&(s=(n?h(e,a,s):h(s))||s);return n&&s&&K(e,a,s),s};const b={open:!1,plugin:"",id:"",label:"",config:{},idError:null,submitError:null};let o=class extends I{constructor(){super(...arguments),this.tab="plugins",this.plugins=[],this.providers=[],this.config={},this.expandedPlugin=null,this.expandedInstance=null,this.revealed=new Set,this.banner=null,this.configFilter="",this.installOpen=!1,this.installSource="",this.installEditable=!1,this.loaded={plugins:!1,advanced:!1},this.drafts={},this.testResults={},this.testing=new Set,this.deleteConfirmId=null,this.rowError={},this.addForm={...b}}createRenderRoot(){return this}connectedCallback(){super.connectedCallback(),this.loadTab(this.tab)}async loadTab(t){try{if(t==="plugins"&&!this.loaded.plugins){const[e,a]=await Promise.all([E(),C().catch(()=>[])]);this.plugins=e,this.providers=a,this.drafts=this.makeDraftsFrom(a),this.loaded={...this.loaded,plugins:!0}}else t==="advanced"&&!this.loaded.advanced&&(this.config=await A(),this.loaded={...this.loaded,advanced:!0})}catch(e){e instanceof p&&(e.status===404||e.status===501)?this.banner={kind:"info",text:"Config API not available on this build — rebuild with PR B to enable."}:this.banner={kind:"error",text:String(e)}}}makeDraftsFrom(t){const e={};for(const a of t)e[a.id]={label:a.label,config:{...a.config}};return e}async refreshProviders(){const t=await C();this.providers=t,this.drafts=this.makeDraftsFrom(t)}switchTab(t){this.tab=t,this.loadTab(t)}instancesFor(t){return this.providers.filter(e=>e.plugin===t.name)}async onSetActive(t){try{this.providers=await j(t),this.drafts=this.makeDraftsFrom(this.providers),this.banner={kind:"info",text:`Active provider: ${t}`}}catch(e){const a=e instanceof p?e.detail??e.message:String(e);this.banner={kind:"error",text:a}}}async onTestProvider(t){const e=new Set(this.testing);e.add(t),this.testing=e;try{const a=await U(t);this.testResults={...this.testResults,[t]:{...a,at:Date.now()}},this.banner={kind:a.ok?"info":"error",text:a.ok?`${t}: ok (${a.latency_ms}ms)`:`${t}: ${a.error??"failed"}`}}catch(a){const n=a instanceof p?a.detail??a.message:String(a);this.testResults={...this.testResults,[t]:{ok:!1,latency_ms:0,error:n,at:Date.now()}},this.banner={kind:"error",text:n}}finally{const a=new Set(this.testing);a.delete(t),this.testing=a}}statusFor(t){const e=this.testResults[t];return e?e.ok?{kind:"connected",title:`Connected (${e.latency_ms}ms)`}:{kind:"failed",title:e.error??"Failed"}:{kind:"untested",title:"Untested"}}onDraftLabelChange(t,e){const a=this.drafts[t];a&&(this.drafts={...this.drafts,[t]:{...a,label:e}})}onDraftConfigChange(t,e,a){const n=this.drafts[t];n&&(this.drafts={...this.drafts,[t]:{...n,config:{...n.config,[e]:a}}})}computePatch(t,e){const a={};e.label!==t.label&&(a.label=e.label);const n={};for(const[s,i]of Object.entries(e.config))JSON.stringify(i)!==JSON.stringify(t.config[s])&&(n[s]=i);return Object.keys(n).length>0&&(a.config=n),a}async onSaveRow(t){const e=this.providers.find(s=>s.id===t),a=this.drafts[t];if(!e||!a)return;const n=this.computePatch(e,a);if(Object.keys(n).length===0){this.banner={kind:"info",text:"No changes to save."};return}try{const s=await L(t,n);this.providers=this.providers.map(l=>l.id===t?s:l),this.drafts={...this.drafts,[t]:{label:s.label,config:{...s.config}}},this.rowError={...this.rowError,[t]:""};const{[t]:i,...h}=this.testResults;this.testResults=h,this.revealed=new Set(Array.from(this.revealed).filter(l=>!l.startsWith(`providers.${t}.`))),this.banner={kind:"info",text:`Saved ${t}`}}catch(s){const i=s instanceof p?s.detail??s.message:String(s);this.rowError={...this.rowError,[t]:i}}}onResetRow(t){const e=this.providers.find(a=>a.id===t);e&&(this.drafts={...this.drafts,[t]:{label:e.label,config:{...e.config}}},this.rowError={...this.rowError,[t]:""})}async onConfirmDelete(t){try{await N(t),this.providers=this.providers.filter(i=>i.id!==t);const{[t]:e,...a}=this.drafts;this.drafts=a;const{[t]:n,...s}=this.testResults;this.testResults=s,this.revealed=new Set(Array.from(this.revealed).filter(i=>!i.startsWith(`providers.${t}.`))),this.deleteConfirmId=null,this.banner={kind:"info",text:`Deleted ${t}`},this.expandedInstance===t&&(this.expandedInstance=null)}catch(e){const a=e instanceof p?e.detail??e.message:String(e);this.rowError={...this.rowError,[t]:a},this.deleteConfirmId=null}}async onRevealToggle(t,e){const a=`providers.${t}.${e}`,n=new Set(this.revealed);if(n.has(a))n.delete(a);else{n.add(a);try{const s=await w(a,!0),i=this.drafts[t];i&&(this.drafts={...this.drafts,[t]:{...i,config:{...i.config,[e]:s.value}}})}catch{}}this.revealed=n}openAddInstance(t){this.addForm={...b,open:!0,plugin:t,id:this.suggestInstanceId(t)}}suggestInstanceId(t){const e=t.replace(/_/g,"-"),a=new Set(this.providers.map(n=>n.id));if(!a.has(e))return e;for(let n=2;n<100;n++){const s=`${e}-${n}`;if(!a.has(s))return s}return e}onAddFormChange(t){this.addForm={...this.addForm,...t,submitError:null}}onAddIdChange(t){const e=t.trim(),a=e&&!G(e)?"id must be 3-64 lowercase alphanumeric characters / dashes; no dots.":null;this.addForm={...this.addForm,id:e,idError:a,submitError:null}}onAddConfigChange(t,e){this.addForm={...this.addForm,config:{...this.addForm.config,[t]:e},submitError:null}}async onAddSubmit(){const t=this.addForm;if(!t.plugin){this.addForm={...t,submitError:"Pick a backing plugin."};return}if(!t.id){this.addForm={...t,submitError:"Enter an instance id."};return}if(t.idError){this.addForm={...t,submitError:t.idError};return}const e={plugin:t.plugin,id:t.id};t.label&&(e.label=t.label),Object.keys(t.config).length>0&&(e.config=t.config);try{const a=await _(e);await this.refreshProviders(),this.expandedInstance=a.id,this.addForm={...b},this.banner={kind:"info",text:`Created ${a.id}`}}catch(a){const n=a instanceof p?a.detail??a.message:String(a);this.addForm={...t,submitError:n}}}async onPluginToggle(t,e){try{const a=await F(t.name,{enabled:e});this.plugins=this.plugins.map(n=>n.name===t.name?{...n,...a}:n)}catch(a){this.banner={kind:"error",text:String(a)}}}async onPluginRemove(t){if(confirm(`Remove plugin ${t}?`))try{await T(t),this.plugins=this.plugins.filter(e=>e.name!==t),this.banner={kind:"info",text:`Removed ${t}`}}catch(e){this.banner={kind:"error",text:String(e)}}}async onInstallSubmit(){const t=this.installSource.trim();if(t)try{await P(t,this.installEditable),this.installOpen=!1,this.installSource="",this.installEditable=!1,this.loaded={...this.loaded,plugins:!1},await this.loadTab("plugins"),this.banner={kind:"info",text:`Queued install for ${t}`}}catch(e){this.banner={kind:"error",text:String(e)}}}async onConfigPatch(t,e){try{await D(t,e),this.config={...this.config,[t]:e}}catch(a){this.banner={kind:"error",text:String(a)}}}async onConfigDelete(t){if(confirm(`Delete ${t}?`))try{await O(t);const e={...this.config};delete e[t],this.config=e}catch(e){this.banner={kind:"error",text:String(e)}}}async onAdvancedRevealToggle(t){const e=new Set(this.revealed);if(e.has(t))e.delete(t);else{e.add(t);try{const a=await w(t,!0);this.config={...this.config,[t]:a.value}}catch{}}this.revealed=e}render(){return r`
			<section class="yaya-settings">
				...
				${this.banner?r`<div class="yaya-banner yaya-banner-${this.banner.kind}" @click=${()=>{this.banner=null}}>${this.banner.text}</div>`:u}
				<div class="yaya-settings-body">
					${this.tab==="plugins"?this.renderPlugins():u}
					${this.tab==="advanced"?this.renderAdvanced():u}
				</div>
				${this.addForm.open?this.renderAddInstance():u}
				${this.deleteConfirmId?this.renderDeleteConfirm(this.deleteConfirmId):u}
			</section>
				...
				<div class="yaya-row-head">
					...
					<label class="yaya-toggle">
						<input
							type="checkbox"
							.checked=${a}
							@change=${h=>this.onPluginToggle(t,h.target.checked)}
						/>
						<span>${a?"enabled":"disabled"}</span>
					</label>
					${i?r`<span class="yaya-row-meta">${i}</span>`:u}
		...
		`}renderPluginBody(t,e){return t.category==="llm-provider"?r`<div class="yaya-row-body">
			...
			${v({schema:t.config_schema??null,values:t.current_config??{},revealSecrets:this.revealed,onToggleReveal:a=>{this.onAdvancedRevealToggle(`plugin.${t.name}.${a}`)},onChange:(a,n)=>{this.onConfigPatch(`plugin.${t.name}.${a}`,n)}})}
		</div>`}renderInstanceRow(t){const e=this.expandedInstance===t.id,a=this.drafts[t.id]??{label:t.label,config:{...t.config}},n=this.statusFor(t.id),s=this.testing.has(t.id),i=this.rowError[t.id],h=new Set(Array.from(this.revealed).filter(l=>l.startsWith(`providers.${t.id}.`)).map(l=>l.slice(`providers.${t.id}.`.length)));return r`
			<li class="yaya-row yaya-instance" data-instance-id=${t.id}>
				...
				${e?r`<div class="yaya-row-body">
							...
							${v({schema:t.config_schema??null,values:a.config,revealSecrets:h,onToggleReveal:l=>{this.onRevealToggle(t.id,l)},onChange:(l,$)=>this.onDraftConfigChange(t.id,l,$)})}
							${i?r`<p class="yaya-row-error">${i}</p>`:u}
							<div class="yaya-row-actions">
		...
		`}renderDeleteConfirm(t){return r`
			...
			<div class="yaya-modal" @click=${()=>{this.addForm={...b}}}>
				<div class="yaya-modal-card" @click=${a=>a.stopPropagation()}>
					...
					<label class="yaya-form-field">
						...
						/>
						${this.addForm.idError?r`<span class="yaya-row-error">${this.addForm.idError}</span>`:u}
					</label>
					<label class="yaya-form-field">
					...
					${e?v({schema:e,values:this.addForm.config,revealSecrets:new Set,onToggleReveal:()=>{},onChange:(a,n)=>this.onAddConfigChange(a,n)}):u}
					${this.addForm.submitError?r`<p class="yaya-row-error">${this.addForm.submitError}</p>`:u}
					<div class="yaya-modal-actions">
						<button
